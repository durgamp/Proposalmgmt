...
export class CostService {
  ...
  private get stageRepo() { return AppDataSource.getRepository(ProjectStageEntity); }
  private get activityRepo() { return AppDataSource.getRepository(ProjectActivityEntity); }
  ...
  async saveTimeline(proposalId: string, dto: BulkSaveStagesDto): Promise<{
    ...
    const { stages, activities } = await AppDataSource.transaction(async (manager) => {
      // Atomic delete + insert — activities first (FK dependency on stages)
      ...
      const savedStages = await manager.save(
        ...
        dto.stages.map((s, idx) =>
          manager.create(ProjectStageEntity, {
            id: s.id ?? uuid(),
            proposalId,
            name: s.name,
      ...
      const savedActivities = await manager.save(
        ...
        dto.activities.map((a, idx) => {
          const entity = manager.create(ProjectActivityEntity, {
            id: a.id ?? uuid(),
            proposalId,
            stageId: a.stageId,
            name: a.name,
          ...
          entity.dependencies = a.dependencies;
          return entity;
        }),
      );
    ...
    await auditService.log({
      ...
      details: `Timeline updated — ${stages.length} stages, ${activities.length} activities`,
    });
